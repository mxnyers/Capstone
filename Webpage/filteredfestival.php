...
<div class="search_by">
    ...
    if(isset($_POST['dateorder'])){
        if($_POST['dateorder'] == 'dateasc'){
            ...
            $sql = "SELECT * FROM Festival ORDER BY StartDate asc";
            ...
            while ($row = mysqli_fetch_assoc($result)){
             ...
             $sql = "SELECT * FROM Festival ORDER BY StartDate desc";
             $result = mysqli_query($conn,$sql);
            $queryresults = mysqli_num_rows($result);
            if($queryresults > 0){
            while ($row = mysqli_fetch_assoc($result)){
        ...
        }elseif($_POST['search'] = 'statesearch'){
             //Run query for datedesc
             $state = $_POST['search'];
             $finalstate=strtoupper($state);
             $sql = "SELECT * FROM Festival where State = '.$finalstate.' ";
             $result = mysqli_query($conn,$sql);
            $queryresults = mysqli_num_rows($result);
            if($queryresults > 0){
            while ($row = mysqli_fetch_assoc($result)){
        ...
        $query = "SELECT * FROM Festival ORDER BY Festivalid asc";
}
    ?>
</div>
</body>
